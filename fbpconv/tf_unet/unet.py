...
from tf_unet.layers import (weight_variable, weight_variable_devonc, bias_variable,
        conv2d, deconv2d,avg_pool, max_pool, crop_and_concat, pixel_wise_softmax_2,
        cross_entropy, rsnr_tf)
...
#    if summaries:
        ...
        #tf.summary.image('summary_output', get_image_summary(output_map))
...
class Trainer(object):
    ...
    def _initialize(self, training_iters, output_path, restore, prediction_path):
        ...
        with tf.variable_scope('t'):
            ...
            self.tstep= tf.Variable(0, trainable=False, name='tstep')
    ...
    def train(self, data_provider, data_provider_test, output_path, output_path_test, training_iters=10, epochs=3, display_step=1, restore=False, write_graph=False, prediction_path = 'prediction'):
        """
        ...
        sess_config = tf.ConfigProto(log_device_placement=False, allow_soft_placement=False)
    ...
    def store_prediction(self, sess, batch_x, batch_y, name):
        ...
        loss, rsnr_ins = sess.run([self.net.cost,self.net.rsnr], feed_dict={self.net.x: batch_x, self.net.y: batch_y, self.net.keep_prob: 1.})

        logging.info("Verification rsnr= {:.1f}, loss= {:.4f}".format(np.average(rsnr_ins), loss))
    ...
    def output_minibatch_stats(self, sess, summary_writer, step, batch_x, batch_y):
        ...
        summary_str, loss, rsnr_ins,predictions = sess.run([self.summary_op,
                                                            self.net.cost,
                                                            self.net.rsnr,
                                                            self.net.predicter],
                                                           feed_dict={self.net.x: batch_x,
        ...
        logging.info("Iter {:}, Minibatch Loss= {:.4f}, Training accuracy (rsnr)= {:.1f}".format(step, loss, np.average(rsnr_ins)))
