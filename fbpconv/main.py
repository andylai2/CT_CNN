...
if __name__ == '__main__':
    ...
    if conf.is_training:
        ...
        x_test, y_test = data_loader_test(25)
        prediction = net.predict(save_path, x_test)
        
        for i in range(25):
            imsave('prediction/x_test_'+ str(i) +'.png', x_test[i,:,:,0])
            imsave('prediction/prediction_'+ str(i) +'.png', prediction[i,:,:,0])
            imsave('prediction/y_test_'+ str(i) +'.png', y_test[i,:,:,0])
            print("FBP: Testing avg RSNR: {:.2f}".format(layers.rsnr(x_test[i], y_test[i])))
            print("FBPConvNet: Testing avg RSNR: {:.2f}".format(layers.rsnr(prediction[i], y_test[i])))

        print("FBP: Testing overall avg RSNR: {:.2f}".format(layers.rsnr(x_test, y_test)))
        print("FBPConvNet: Testing overall avg RSNR: {:.2f}".format(layers.rsnr(prediction, y_test)))